...
> Машиночитаемая спецификация: [openapi.yaml](openapi.yaml) (OpenAPI 3.1).
> Открыть в Swagger UI / Redoc или импортировать в Postman.
...
**Выпуск и отзыв — в админ-панели:** «Настройки сайта» → секция «API для внешней
программы». Кнопка «Выпустить API-токен» в шапке страницы создаёт клиента и один раз
показывает токен (с кнопкой «Копировать»). В списке клиентов доступны «Отозвать»
(доступ прекращается, запись остаётся) и «Удалить».
...
## Проверка API (для админов/разработчиков)

Artisan-команда `api:check` — smoke-тест всех эндпоинтов: выпускает временный
токен, проверяет коды ответов (401/200/404/422) и форму JSON, чистит за собой.
Данные **не изменяются** (POST проверяется только на валидацию). Код возврата
`0` — все проверки пройдены, `1` — есть провал (удобно для CI).

Запускать в контейнере, который отдаёт HTTP (там `config('app.url')` = свой
веб-сервер):
...
Опции: `--base-url=` (другой адрес сервера), `--regatta=<external_id>` (регата для
проверок чтения; по умолчанию — с наибольшим числом заявок), `--keep-token`.
