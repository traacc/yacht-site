 ...
 * регата берётся из него. Гонки (RegattaEvents) создаются из метаданных файла по
 * имени (идемпотентно).
 *
 * Якорь участника — ЯХТА (парусный номер vfps_number): заявка и строка итога ищутся
 * и создаются по yacht_id (в рамках регаты/результата), поэтому результаты всегда
 * привязаны к конкретной лодке, а не к имени клуба. $createMissing управляет только
 * созданием недостающих ЯХТ. Команда (обязательна по схеме) — заглушка на яхту:
 * берётся из «Команда/Спонсор» файла или имени яхты и создаётся при необходимости
 * всегда, с защитой от коллизий (клуб под несколькими лодками → суффикс парусного №).
 * Идемпотентно: повторный запуск обновляет записи той же яхты, не плодя дублей.
 *
 * @see ImportRegattaResultItemsAction — аналогичный импорт из CSV (только итоги, по имени).
 */
class ImportRgdResultItemsAction
{
    public function __construct(private readonly RgdParser $parser)
    {
    }
