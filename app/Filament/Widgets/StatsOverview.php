...
class StatsOverview extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        $newEntriesCount = RegattaEntry::where('status', 'pending')
            ->whereHas('regatta', fn ($q) => $q
                ->visibleForUser()
                ->where('date_end', '>=', now()->toDateString()))
            ->count();

        $stats = [
            $this->createCustomStat('Регаты', Regatta::query()->visibleForUser()->count(), 'dashboard_regatta_count', 'text-blue-500/10', RegattaResource::getUrl('index')),
        ...
        // Общие по ассоциации счётчики ведут в разделы, закрытые для админа-разработчика.
