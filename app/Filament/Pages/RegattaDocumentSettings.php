...
/**
 * Страница настроек обязательных документов для регат.
 *
 * Переключатели формируются динамически из таблицы yacht_document_types
 * (только типы с is_configurable = true).
 */
class RegattaDocumentSettings extends Page
{
    protected string $view = 'filament-panels::pages.page';

    protected static string|BackedEnum|null $navigationIcon = Heroicon::OutlinedDocumentText;

    protected static ?string $navigationLabel = 'Документы регат';

    protected static ?string $title = 'Обязательные документы регат';
    ...
    public function mount(UpdateRegattaRequiredDocumentsAction $action): void
    {
    ...
    public function form(Schema $schema): Schema
    {
        ...
        return $schema
            ...
            ->components([
                Section::make('Обязательные документы')
                    ->description('Документы, которые требуются при создании регаты.')
                    ->schema(
                        $types->map(
    ...
    public function save(UpdateRegattaRequiredDocumentsAction $action): void
    {
        ...
        Notification::make()
            ...
            ->body('Изменения применены. Теперь при создании регаты будет требоваться обновлённый список документов.')
            ->success()
            ->send();
    }
}
