    ...
    <div x-show="isOpen" 
        ...
        <div x-show="isOpen" 
            ...
            <form wire:submit.prevent="sendResetLink" class="space-y-4 mt-2" x-show="tab === 'forgot'">
                ...
                @if($resetLinkSent)
                    <div class="text-sm text-green-600 bg-green-50 p-3 rounded">
                        Если не знаете свои регистрационные данные, отправьте запрос администратору на их восстановление
            ...
            <form wire:submit.prevent="sendLoginCredentials" class="space-y-4 mt-2" x-show="tab === 'send'"
                ...
                <div x-data="{ show: false }" class="relative">
                    <input :type="show ? 'text' : 'password'" id="senderPassword" wire:model="senderPassword" placeholder="Ваш пароль"
                           class="mt-1 block w-full border-0 border-b border-[#EAEAEA] sm:text-sm pr-8 @error('senderPassword') border-red-300 @enderror">
                    <button type="button" @click="show = !show"
                            class="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 focus:outline-none">
                        <svg x-show="!show" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
