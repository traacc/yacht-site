<!DOCTYPE html>
<html lang="ru">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
    <title>История команды — {{ $team->name }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
            box-sizing: border-box;
        }

        body {
            font-family: "DejaVu Sans", sans-serif;
            font-size: 11px;
            color: #1a1a2e;
            background: #ffffff;
            padding: 24px 32px;
        }

        /* ── Шапка ── */
        .header {
            display: flex;
            justify-content: space-between;
            align-items: flex-end;
            border-bottom: 3px solid #2E325C;
            padding-bottom: 12px;
            margin-bottom: 20px;
        }

        .header-left .title {
            font-size: 20px;
            font-weight: bold;
            color: #2E325C;
        }

        .header-left .subtitle {
            font-size: 12px;
            color: #6b7280;
            margin-top: 2px;
        }

        .header-right .generated {
            font-size: 8px;
            color: #9ca3af;
            margin-top: 4px;
        }

        /* ── Блок информации о команде ── */
        .section-title {
            font-size: 13px;
            font-weight: bold;
            color: #2E325C;
            margin: 18px 0 8px;
        }

        .info-table td {
            padding: 4px 10px 4px 0;
            font-size: 10px;
            vertical-align: top;
        }

        .info-table td.label {
            color: #6b7280;
            width: 150px;
        }

        .info-table td.value {
            color: #1f2937;
            font-weight: bold;
        }

        .description {
            font-size: 10px;
            color: #374151;
            margin-top: 8px;
            line-height: 1.5;
        }

        /* ── Таблицы ── */
        table.data {
            width: 100%;
            border-collapse: collapse;
            margin-top: 4px;
        }

        table.data thead tr {
            background-color: #2E325C;
            color: #ffffff;
        }

        table.data thead th {
            padding: 8px 10px;
            text-align: left;
            font-size: 9.5px;
            font-weight: bold;
            text-transform: uppercase;
            letter-spacing: 0.5px;
            white-space: nowrap;
        }

        table.data thead th.center {
            text-align: center;
        }

        table.data tbody td {
            padding: 7px 10px;
            font-size: 10px;
            color: #1f2937;
            border-bottom: 1px solid #e5e7eb;
        }

        table.data tbody td.center {
            text-align: center;
        }

        table.data tbody tr:nth-child(even) td {
            background-color: #f7f9fc;
        }

        .muted {
            color: #9ca3af;
        }

        /* ── Подвал ── */
        .footer {
            margin-top: 18px;
            padding-top: 10px;
            border-top: 1px solid #e5e7eb;
            text-align: right;
        }

        .footer .note {
            font-size: 8px;
            color: #9ca3af;
        }
    </style>
</head>
<body>

    {{-- Шапка --}}
    <div class="header">
        <div class="header-left">
            <div class="title">История команды</div>
            <div class="subtitle">{{ $team->name }}</div>
        </div>
        <div class="header-right">
            <div class="generated">Сформировано: {{ now()->isoFormat('D MMMM YYYY') }}</div>
        </div>
    </div>

    {{-- Информация о команде --}}
    <div class="section-title">Информация о команде</div>
    <table class="info-table">
        <tr>
            <td class="label">ID</td>
            <td class="value">{{ $team->getFormattedExternalIdAttribute() }}</td>
        </tr>
        <tr>
            <td class="label">Капитан</td>
            <td class="value">{{ $captain }}</td>
        </tr>
        <tr>
            <td class="label">Дата регистрации</td>
            <td class="value">{{ $team->created_at?->format('d.m.Y') ?? '—' }}</td>
        </tr>
        <tr>
            <td class="label">Статус</td>
            <td class="value">{{ $status }}</td>
        </tr>
        <tr>
            <td class="label">Рейтинг</td>
            <td class="value">{{ $rating }}</td>
        </tr>
        <tr>
            <td class="label">Участие в регатах</td>
            <td class="value">{{ $participation->count() }}</td>
        </tr>
    </table>

    @if(filled($team->description))
        <div class="description">{{ $team->description }}</div>
    @endif

    {{-- Состав команды --}}
    <div class="section-title">Состав команды</div>
    <table class="data">
        <thead>
            <tr>
                <th>Участник</th>
                <th style="width: 110px;">Дата рождения</th>
                <th class="center" style="width: 90px;">Разряд</th>
                <th class="center" style="width: 90px;">Роль</th>
            </tr>
        </thead>
        <tbody>
            @forelse($members as $member)
                <tr>
                    <td>{{ $member['name'] }}</td>
                    <td>{{ $member['birthday'] }}</td>
                    <td class="center">{{ $member['category'] }}</td>
                    <td class="center">{{ $member['role'] }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="4" class="muted">Нет данных</td>
                </tr>
            @endforelse
        </tbody>
    </table>

    {{-- Участие в регатах --}}
    <div class="section-title">Участие в регатах</div>
    <table class="data">
        <thead>
            <tr>
                <th>Регата</th>
                <th style="width: 130px;">Дата регаты</th>
                <th style="width: 110px;">Яхта</th>
                <th class="center" style="width: 60px;">Место</th>
            </tr>
        </thead>
        <tbody>
            @forelse($participation as $entry)
                <tr>
                    <td>{{ $entry['regatta'] }}</td>
                    <td>{{ $entry['date_event'] }}</td>
                    <td>{{ $entry['yacht'] }}</td>
                    <td class="center">{{ $entry['place'] }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="4" class="muted">Нет данных</td>
                </tr>
            @endforelse
        </tbody>
    </table>

    {{-- Подвал --}}
    <div class="footer">
        <div class="note">Документ сформирован автоматически</div>
    </div>

</body>
</html>
