...
- **Services** (`app/Services`): `RaceScorer` и `RatingCalculator` (подсчёт очков и рейтингов, dense ranking по `rank_position`), `RegattaService`, `SettingsService` (настройки страниц из админки), `Rgd/RgdParser` и `RgdParticipantsExporter` (обмен с судейской программой), интеграции — `TelegramService`, `VkService`, `FlashCallService` (подтверждение телефона звонком, zvonok.com), `WeatherService`, `YandexGeocoderService`, `YandexMapService`, а также `ImageConverter`, `SitemapGenerator`, `TeamRoleGuard`, `Finance/FinancialReportBuilder` (финансовый отчёт за период по реестру платежей: приходы, отдельный итог по разделу «Услуги», выгрузка в Excel).
...
- **Прочее**: `Exports/` (Excel: результаты, участники регаты, команды, пользователи, яхты, реестр платежей, финансовый отчёт за период), `Imports/YachtImport`, `Policies/TeamPolicy`, `Rules/YandexCaptcha`, `Support/` (`AccessControl`, `SafeDelete`, `Svg`), `Http/Middleware` (`MaintenanceMode`, `FilamentAuthenticate`, `VerifyApiToken`).
